  ...
  public LoadResult load(PartitionLoadArgs loadArgs) throws PartitionLoadingException {
    ...
    try {
      FileMetadataLoader fml = new FileMetadataLoader(partDir_, loadArgs.isRecursive(),
          getFileDescriptors(),
          getTable().getHostIndex(), loadArgs.getValidTxnList(),
          loadArgs.getValidWriteIdList());
      ...
      if (isMarkedCached()) fml.setForceRefreshBlockLocations(true);
      fml.load();
      setFileDescriptors(fml.getLoadedFds());
    } catch (IOException e) {
