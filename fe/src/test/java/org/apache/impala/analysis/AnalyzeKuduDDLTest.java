...
"Number of specified range partition values is different than the number of " +
  ...
  "Column 'b' in 'RANGE (b) (PARTITION VALUE = 'abc')' is not a key column. " +
    ...
    AnalysisError("create table ts_ranges (ts timestamp primary key, i int)" +
        ...
        "compatible with partitioning column 'ts' (type: TIMESTAMP).", isExternalPurgeTbl);
  ...
  public void TestAlterKuduTable() {
    ...
    // altering the external.table.purge property should be allowed.
    AnalyzesOk("alter table functional_kudu.alltypes set tblproperties ('external.table"
        + ".purge'='FALSE'");
    AnalyzesOk("alter table functional_kudu.alltypes set tblproperties ('external.table"
        + ".purge'='TRUE'");
