...
public abstract class CatalogDdlOperation extends CatalogOperation {

  private static final Logger LOG = LoggerFactory.getLogger(CatalogDdlOperation.class);
  ...
  public CatalogDdlOperation(TDdlExecRequest ddlExecRequest, TDdlExecResponse response,
   ...
   */
  protected void dropColumnStats(Table table) throws ImpalaRuntimeException {
    Preconditions.checkState(table.isWriteLockedByCurrentThread());
  ...
  protected void applyAlterPartition(Table tbl, HdfsPartition.Builder partBuilder)
    ...
    try (MetaStoreClient msClient = catalog_.getMetaStoreClient()) {
      ...
      applyAlterHmsPartitions(tbl.getMetaStoreTable().deepCopy(), msClient,
          tbl.getTableName(), Collections.singletonList(hmsPartition));
