...
public class HdfsTable extends Table implements FeFsTable {
  ...
  // metric used to track the time taken to load filemetadata when there is a filemetadata
  // cache miss for a given validWriteIdList
  public static final String FILEMETADATA_CACHE_MISS_DURATION =
      "filemetadata-cache-miss-load-duration";
  ...
  public TGetPartialCatalogObjectResponse getPartialInfo(
    ...
    Map<HdfsPartition, TPartialPartitionInfo> missingPartitions = Maps.newHashMap();
          ...
          try {
            ...
            if (LOG.isDebugEnabled()) {
              LOG.debug(String.format(
                  "Could not use cached file descriptors of partition {} of table {} "
                      + "for writeIdList {}", part.getPartitionName(), getFullName(),
                  reqWriteIdList), ex);
            }
            misses.inc();
            missingPartitions.put(part, partInfo);
          }
        }
    ...
    if (!missingPartitions.isEmpty()) {
      setFileMetadataFromFS(reqWriteIdList, missingPartitions);
    }
   ...
   */
  private void setFileMetadataFromFS(ValidWriteIdList reqWriteIdList,
      Map<HdfsPartition, TPartialPartitionInfo> partToPartialInfoMap)
      throws CatalogException {
    Timer.Context timer = metrics_.getTimer(FILEMETADATA_CACHE_MISS_DURATION).time();
    try {
      String logPrefix = String.format(
          "Fetching file and block metadata for %s paths for table %s for "
              + "validWriteIdList %s", partToPartialInfoMap.size(), getFullName(),
          reqWriteIdList);
      ...
      Map<HdfsPartition, List<FileDescriptor>> fdsByPart = new ParallelFileMetadataLoader(
          this, partToPartialInfoMap.keySet(), reqWriteIdList, validTxnList, logPrefix)
          .loadAndGet();
  ...
  public void initMetrics() {
    ...
    metrics_.addTimer(FILEMETADATA_CACHE_MISS_DURATION);
  }
