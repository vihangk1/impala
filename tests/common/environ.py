...
class ImpalaTestClusterProperties(object):
  ...
  def runtime_flags(self):
    ...
    if self._runtime_flags is None:
      self._runtime_flags = ImpalaTestClusterProperties.__get_flag_from_webui(self, self._web_ui_url)
  ...
  def catalog_runtime_flags(self):
    ...
    if self._catalog_runtime_flags is None:
      self._catalog_runtime_flags = ImpalaTestClusterProperties.__get_flag_from_webui(self, self._catalog_web_ui_url)
  ...
  def is_event_polling_enabled(self):
    ...
    try:
      ...
      return key in self.catalog_runtime_flags and int(self.catalog_runtime_flags[key]["current"]) > 0
    except Exception:
      if self.is_remote_cluster():
        # IMPALA-8553: be more tolerant of failures on remote cluster builds.
        LOG.exception("Failed to get flags from web UI, assuming catalog V1")
